...
filament::Camera* filamentAppMainCamera = nullptr;
...
void FilamentApp::run(const Config& config, SetupCallback setupCallback,
    ...
    filamentAppMainCamera = window->mMainCamera;
            ...
            switch (event.type) {
                ...
                case SDL_MOUSEWHEEL:
                    if (!io || !io->WantCaptureMouse)
                        window->mouseWheel(event.wheel.y * 16);
                    break;
                case SDL_MOUSEBUTTONDOWN:
                    if (!io || !io->WantCaptureMouse)
...
FilamentApp::Window::Window(FilamentApp* filamentApp,
    ...
    mMainCamera->lookAt({1, 1, 1}, {0, 0, 0}, {0, 1, 0});
}
